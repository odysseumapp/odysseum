...
const props = defineProps<{ arcs: DocumentSummary[]; documents: DocumentSummary[]; busy: boolean }>()
...
const members = (arc: string) => props.documents.filter(doc => positions(doc)[arc] !== undefined)
  .sort((a, b) => positions(a)[arc]! - positions(b)[arc]! || a.id.localeCompare(b.id))
const available = computed(() => props.documents.filter(doc => adding.value && positions(doc)[adding.value.id] === undefined))
const columns = computed(() => Math.max(5, ...props.documents.flatMap(doc => Object.values(positions(doc)).map(value => Math.ceil(value) + 2))))
...
function attach() {
  const document = props.documents.find(doc => doc.id === chosen.value)
  if (!document || !adding.value || !Number.isFinite(position.value) || position.value < 1 || position.value > 10001) return
...
function drop(event: DragEvent, arc: string) {
  ...
  const document = props.documents.find(doc => doc.id === dragged.value!.id)
...
<template>
  <div class="arcs-view">
    <header class="arcs-heading">
      <div><span class="eyebrow">THREADS THROUGH YOUR STORY</span><h1>Story arcs</h1><p>Give each thread its own line. Place documents where they belong.</p></div>
      <button class="primary-button" @click="emit('create')"><Plus :size="16" />New arc</button>
    </header>
    <div v-if="!arcs.length" class="arcs-empty"><GitBranch :size="34" /><h2>Every story has its threads.</h2><p>Add an arc for a theme, a relationship, or a journey.<br />Then place documents along its line.</p></div>
    <template v-else>
      ...
      <div class="arcs-scroll" tabindex="0" role="region" aria-label="Story arc timelines">
        ...
        <section v-for="(lane, index) in lanes" :key="lane.arc.id" class="arc-lane" :aria-label="`${lane.arc.title} arc`" :style="{ width: `${width + 176}px`, '--arc-color': ['#78845b', '#a97b5f', '#77959a', '#a086a0', '#a99a61'][index % 5] }">
          <div class="arc-label"><button class="arc-name" :title="`Edit ${lane.arc.title}`" @click="emit('open', lane.arc)">{{ lane.arc.title }}<ArrowUpRight :size="12" /></button><span>{{ lane.items.length }} {{ lane.items.length === 1 ? 'document' : 'documents' }}</span><button class="arc-add" :aria-label="`Add document to ${lane.arc.title}`" :disabled="busy" @click="add(lane.arc)"><Plus :size="13" />Add document</button></div>
      ...
      <p class="arcs-footnote">A document can belong to several arcs. Each line keeps its own arrangement.</p>
    </template>
    <AppDialog v-if="adding" :title="`Add to ${adding.title}`" @close="adding = null"><form @submit.prevent="attach"><label class="field-label" for="arc-document">Document</label><select id="arc-document" v-model="chosen" required><option disabled value="">Choose a document</option><option v-for="document in available" :key="document.id" :value="document.id">{{ document.title }} — {{ document.folder || 'Project root' }}</option></select><p v-if="!available.length" class="field-help">All available documents are already on this arc. Create a document in another section to add more.</p><label class="field-label" for="arc-position">Position</label><input id="arc-position" v-model.number="position" type="number" min="1" max="10001" step="1" required /><div class="dialog-actions"><button type="button" class="small-button" @click="adding = null">Cancel</button><button class="primary-button" :disabled="!chosen || busy">Add to arc</button></div></form></AppDialog>
  </div>
</template>
