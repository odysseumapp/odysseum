...
<template>
  <div class="document-link-picker" :class="`${kind}-picker`" role="group" :aria-label="kind === 'arc' ? 'Arcs for this beat' : `${label} in this scene`">
    ...
    <p v-if="!documents.length" class="field-help">No {{ label.toLowerCase() }} yet. Add one to make it available for every {{ kind === 'arc' ? 'beat' : 'scene' }}.</p>
  </div>
</template>
